...
export function ProcessTimeline() {
    ...
    return (
        <section ref={containerRef} className="relative py-32 md:py-40 px-6 bg-charcoal-dark min-h-screen">
            <div className="max-w-4xl mx-auto relative">
                <h2 className="text-3xl md:text-5xl font-serif font-bold text-cloud text-center mb-24 tracking-tight uppercase">
                ...
                <div className="space-y-28 md:space-y-36">
                    {milestones.map((ms, index) => (
                        ...
                        >
                            <div className={`md:w-1/2 p-6 md:p-8 ${index % 2 === 0 ? "md:pl-16 text-center md:text-left" : "md:pr-16 text-center md:text-right"}`}>
                                <span className="text-2xl md:text-4xl font-mono font-bold text-accent-gold/60 mb-2 block tracking-wider">{ms.year}</span>
                                <h3 className="text-lg md:text-2xl font-serif font-semibold text-cloud mb-3">{ms.title}</h3>
                                <p className="text-cloud/70 leading-relaxed font-sans text-sm md:text-base">{ms.description}</p>
                            </div>

                            {/* Dot on line */}
                            <div className="absolute left-1/2 -translate-x-1/2 top-12 w-3.5 h-3.5 rounded-full border-2 border-accent-gold bg-charcoal-dark z-10 hidden md:block" />
                        </div>
                    ))}
                </div>
            </div>
